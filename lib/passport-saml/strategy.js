...
Strategy.prototype.authenticate = function (req) {
  ...
  saml.parse(assertion, function (err, profile) {
    ...
    // get the org
    self._orgRepository.find({ idprovider_id : profile.issuer })
      .then(function (orgs) {
            ...
            if (properties && properties.identity_management && properties.identity_management.saml) {
              ...
              saml.validate(assertion, {
                ...
                if (self._passReqToCallback) {
                  self._verify(req, profile, issuer, claims, username, email, company, firstname, lastname, org, verified);
                } else {
                  self._verify(profile, issuer, claims, username, email, company, firstname, lastname, org, verified);
                }
